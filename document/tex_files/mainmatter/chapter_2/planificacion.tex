...
El proyecto cuenta con tres entregables finales: El motor Hephaestus, un juego didáctico y un conjunto de prácticas para alumnos de segundo curso del Grado de Ingeniería Informática de la Universidad de Valladolid, cursando la asignatura de Sistemas Distribuidos.

En el diagrama de la figura \ref{fig:chap2_pbs_diagram} se muestra el PBS (Project Breakdown Study) del proyecto. Este diagrama muestra las diferentes partes que componen todo el sistema o proyecto y, a su vez, en las subpartes en las que están divididas. Cada una de estas subpartes corresponde a un entregable del proyecto.

Al tratarse de una metodología iterativa cada entregable pasa por varias versiones antes de decantarse por la final. Cada una de estas versiones corresponde a una iteración de ese entregable y produce un prototipo. En la planificación de este proyecto, cada iteración se estima en una duración de entre 1 y 2 semanas de trabajo, esto es debido a las restricciones laborales con las que cuenta el alumno a la hora de completar la iteración. Por eso mismo, y dependiendo de la cantidad de iteraciones necesarias para llegar a la versión final de un entregable, la ejecución del proyecto se alarga sustancialmente a lo largo del tiempo.
...
El último prototipo del entregable es otro documento escrito que sirve como introducción a la sección de las prácticas de la asignatura en la cual este proyecto se emplea.

Siguiendo el formato de los tres documentos didácticos anteriores, se crea la práctica cero en la cual se explica lo que se va a hacer a lo largo de las tres prácticas siguientes, se hace una descripción del videojuego, qué tecnologías se van a emplear durante el desarrollo de dichas prácticas y cómo se pone en marcha todo el proyecto.
...
Mediante este entregable se resuelve el problema de simular un sistema distribuido dentro de la máquina empleada por el alumno, así como la distribución del motor y el juego a los mismos alumnos o a cualquier usuario que así lo requiera. Para ello se hace uso de la tecnología Docker.
...
Se prueba el funcionamiento de la navegación haciendo uso de la biblioteca. Esto es especialmente importante debido a que la biblioteca hace uso de un archivo especial en el cual se tienen que describir las rutas a los archivos creados.
