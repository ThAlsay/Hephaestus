...
Durante el proyecto se emplean tres tipos diferentes de pruebas para los distintos entregables. Debido al tiempo disponible, el rango de actuación y la cantidad de tipos de pruebas realizadas a cada una de las partes se ven disminuidos. Por ello, se prioriza el correcto funcionamiento del sistema y una experiencia como mínimo adecuada para el alumno, se dejan de lado pruebas más profundas y se centra el mayor esfuerzo en el motor, como pieza central del proyecto.

La cantidad de pruebas y profundidad de las mismas se ven afectadas por el tiempo disponible. Por ello, y debido a que la posibilidad de recuperar el sistema y dejarlo en un estado anterior es muy sencilla, gracias al empleo de Docker, se hicieron recortes en este punto del proyecto, siendo las pruebas realizadas suficientes para asegurar el correcto funcionamiento del sistema, pero dejando de lado la integridad del mismo.

\section{Pruebas unitarias}

Las pruebas unitarias en el desarrollo de software sirven para verificar el funcionamiento de las partes más pequeñas del sistema que está siendo probado. Se aseguran de que las partes verificadas se comporten como se espera y no surja ningún error.

Por lo general las pruebas unitarias requieren de un tiempo mayor que otro tipo de pruebas para su implementación ya que, al centrarse en las partes más pequeñas del sistema evaluado, requieren de múltiples pruebas por unidad del sistema de manera que se abarquen todos los escenarios posibles.

Debido a su naturaleza, las pruebas unitarias son muy útiles para probar sistemas o aplicaciones software que normalmente no cuentan con interacción directa del usuario, sino que son empleadas por otros sistemas, por ejemplo las bibliotecas de algún lenguaje. Por ello, y debido al recorte por tiempo ya mencionado, estas pruebas solo se emplean para asegurar el correcto funcionamiento del motor Hephaestus.

Al tratarse de Hephaestus el lenguaje para las pruebas del motor es Elixir, por lo tanto, se hace uso de las herramientas proporcionadas por el propio lenguaje para hacer las pruebas, tal y como se muestra en el ejemplo \ref{cod:ejemplo_test_unitario_elixir}. Mediante estas pruebas se asegura el correcto funcionamiento de las piezas del motor necesarias para crear un videojuego, incluyendo todo lo implementado mediante \textit{GenServer}. Para ello las pruebas se centran en el código que produce algún cambio en el estado, esto es se excluye de las pruebas todo código que no modifique el comportamiento de la biblioteca \textit{GenServer} como puede ser un \textit{getter} de un campo del estado.
\\
...
En total el motor Hephaestus cuenta con 35 pruebas unitarias cuya utilidad se ve reflejada a la hora de realizar iteraciones sobre el código del motor. Al finalizar cada iteración el conjunto total de pruebas no debe fallar y, en caso de que alguna prueba falle, la propia naturaleza de la prueba da una idea de en qué parte del código se ha producido el error. Al finalizar el proyecto todas las pruebas deben ser correctas y no producir ningún error, pues lo contrario significaría la entrega de un producto defectuoso.
...
El juego didáctico actúa como hilo conductor de las prácticas y, por lo tanto, es natural que se enlacen las pruebas end-to-end del mismo con las prácticas proporcionadas a los alumnos, en este caso como solución de las mismas.